...
class ImageCreation:
    ...
    def __init__(self, resource_handler:ResourceHandler, user_data:UserData):
        self.resource_handler = resource_handler
        self.user_data = user_data
    
    def n_lowest_winrates(self, counter:Counter, n:int):
    ...
    def get_predicted_winrate(self, picks:Counter, wins:Counter, wins_total:int, matches_total:int) -> Counter:
        winrate = {}
    ...
    def create_hero_analysis_image(self, user:User, target_hero_code:str, darkmode:bool=False) -> tuple[np.ndarray, int, int, MatchHistory]:
        ...
        axes[0, 0].text(1.0, 1.2, f"{self.resource_handler.hero_list.get_hero_by_code(target_hero_code).name}", fontsize=40, va='center', transform=axes[0, 0].transAxes, color=textcol)
    ...
    def create_match_summary_image(self, user:User, darkmode:bool=False) -> tuple[np.ndarray, MatchHistory]:
    ...
    def create_legend_data_summary_image(self, legend_data:dict, darkmode:bool=False) -> np.ndarray:
        ...
        winrate = self.get_predicted_winrate(picks, wins, int(sum(wins.values())/5), int(sum(picks.values())/5))
        winrate_early_picks = self.get_predicted_winrate(early_picks, early_picks_wins, int(sum(early_picks_wins.values())/5), int(sum(early_picks.values())/5))
        winrate_third_picks = self.get_predicted_winrate(third_picks, third_picks_wins, int(sum(third_picks_wins.values())/5), int(sum(third_picks.values())/5))
        winrate_late_picks = self.get_predicted_winrate(late_picks, late_picks_wins, int(sum(late_picks_wins.values())/5), int(sum(late_picks.values())/5))
    ...
    def create_legend_data_image_one_hero(self, target_hero_code:str, target_hero_name:str, legend_data:dict, darkmode:bool=False) -> tuple[np.ndarray, bool]:
        picks = Counter(legend_data["picks"])
        picks_all = Counter(legend_data["picks_all"])
        wins = Counter(legend_data["wins"])

        early_picks = Counter(legend_data["early_picks"])
        early_picks_all = Counter(legend_data["early_picks_all"])
        early_picks_wins = Counter(legend_data["early_picks_wins"])

        third_picks = Counter(legend_data["third_picks"])
        third_picks_all = Counter(legend_data["third_picks_all"])
        third_picks_wins = Counter(legend_data["third_picks_wins"])

        late_picks = Counter(legend_data["late_picks"])
        late_picks_all = Counter(legend_data["late_picks_all"])
        late_picks_wins = Counter(legend_data["late_picks_wins"])
        ...
        for i in range(5):
            axes[4, i+1].axis('off')
        axes[4, 1].text(0, 0.4, f"{round(100*first_picks_all[target_hero_code]/matches_n,1)}%", fontsize=fontsize_s+2, ha='center', color=textcol)
        axes[4, 1].text(0.8, 0.4, f" - ", fontsize=fontsize_s+2, ha='center', color=textcol)
        axes[4, 4].text(0, 0.4, f"First picked in {first_picks_all[target_hero_code]} out of {matches_n} matches.", fontsize=fontsize_s+2, ha='center', color=textcol)
        ...
        for i in range(5):
            axes[5, i+1].axis('off')
        axes[5, 1].text(0, 0.4, f"{round(100*early_picks_all[target_hero_code]/matches_n,1)}%", fontsize=fontsize_s+2, ha='center', color=textcol)
        axes[5, 1].text(0.8, 0.4, f" - ", fontsize=fontsize_s+2, ha='center', color=textcol)
        axes[5, 4].text(0, 0.4, f"Picked early in {early_picks_all[target_hero_code]} out of {matches_n} matches.", fontsize=fontsize_s+2, ha='center', color=textcol)
        ...
        for i in range(5):
            axes[6, i+1].axis('off')
        axes[6, 1].text(0, 0.4, f"{round(100*third_picks_all[target_hero_code]/matches_n,1)}%", fontsize=fontsize_s+2, ha='center', color=textcol)
        axes[6, 1].text(0.8, 0.4, f" - ", fontsize=fontsize_s+2, ha='center', color=textcol)
        axes[6, 4].text(0, 0.4, f"Picked third in {third_picks_all[target_hero_code]} out of {matches_n} matches.", fontsize=fontsize_s+2, ha='center', color=textcol)
        ...
        for i in range(5):
            axes[7, i+1].axis('off')
        axes[7, 1].text(0, 0.4, f"{round(100*late_picks_all[target_hero_code]/matches_n,1)}%", fontsize=fontsize_s+2, ha='center', color=textcol)
        axes[7, 1].text(0.8, 0.4, f" - ", fontsize=fontsize_s+2, ha='center', color=textcol)
        axes[7, 4].text(0, 0.4, f"Picked late in {late_picks_all[target_hero_code]} out of {matches_n} matches.", fontsize=fontsize_s+2, ha='center', color=textcol)
    ...
    def create_ban_summary_image(self, user:User, darkmode:bool=False) -> tuple[np.ndarray, MatchHistory]:
        matches = user.get_match_data(self.resource_handler.hero_list)
        match_result_vector = matches.get_match_result_vector()
        ...
        own_ban_winrate_counts = self.get_predicted_winrate(own_ban_counts, own_ban_win_counts, sum(match_result_vector), len(match_result_vector))
        enemy_ban_winrate_counts = self.get_predicted_winrate(enemy_ban_counts, enemy_ban_win_counts, sum(match_result_vector), len(match_result_vector))
